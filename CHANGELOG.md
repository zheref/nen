...
- **shu** — `detect` proposes the full `nextjs` row set (declared-only rows as explicit stubs, `coverage` when the manifest declares it, one lane per workspace) and the `gatsby` stack end to end (`gatsby build/develop/serve`, the archive and gh-pages steps, unsupported rows with their reasons) ([#PR](link), closes #116)
