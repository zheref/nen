...
- **shu** — `detect` proposes the full `nextjs` row set (declared-only rows as explicit `unsupported` seats that say what they are, `coverage` when the manifest declares the task, one lane per workspace member) and the `gatsby` stack end to end (`gatsby build/develop/serve`, the archive and gh-pages steps, seats with their reasons). Two cross-checks are stricter with it: a pack token is answered from a declared script only where something CORROBORATES the match beyond word count, and a row is withheld unless the manifest declares the tool its package manager hands the work to (`<pm> <tool>`, `<pm> exec <tool>`, `npx <tool>`) and the runner's own config declares the task a `run <task>` names (`turbo.json`, not `package.json`'s scripts, for `turbo run x`) ([#PR](link), closes #116)
