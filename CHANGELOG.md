...
- **shu** — new family `nen shu` (`detect build test ui-test lint archive release dev run deploy coverage tools warmup`): `detect` proposes a `nen/contract.json` project block from filename markers, cross-checked against the profiles pack and never written without `--write`; the executor runs a lane's declared verbs with `--dry-run` printing every command first and preconditions asserted and never performed. It extends this CLI's published 0/1/2 with three codes: 3 (unsupported host), 4 (unsupported verb for this lane) and 5 (the declared program could not be started). `shu tools` and `shu warmup` are declared, documented and refuse at 4 in this release; the other eleven verbs run ([#PR](link), closes #110)
