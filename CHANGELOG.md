...
- **profiles** — a bundled, inert catalogue of reference commands per stack (`profiles/<stack>.json`, seven stacks) that nothing which spawns a process may read (pinned by a source-scan test), and `docs/STACK-MATRIX.md`, generated from it by `bun run matrix` and drift-checked in the suite ([#PR](link), closes #111)
