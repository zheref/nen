...
export function retarget(seams: Seams, target: Target, prNumber: number, base: string): RetargetResult {
  const result = seams.run(GH, [...retargetArgv(target, prNumber, base)]);
  if (result.code !== 0) {
    return {
      ok: false,
