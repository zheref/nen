// src/epic/verb.ts -- `nen epic next-wave`.
//
...
export const epicVerb: Verb = {
  ...
  run(context: VerbContext): number {
    ...
    if (subcommand !== "next-wave") {
      return usage(
        context.io,
        `unknown 'epic' subcommand '${subcommand ?? "(none)"}'. Try 'epic next-wave'.`,
      );
    }
    const bodyFile = context.values["body-file"];
    if (bodyFile === undefined) return usage(context.io, "--body-file <path> is required.");
    const citation = context.values["citation"];
    ...
    const completedRaw = context.values["completed"];
    const completed =
      completedRaw === undefined ? null : Number(completedRaw.replace(/^#/, ""));
    if (completed !== null && !Number.isInteger(completed)) {
      return usage(context.io, `--completed '${completedRaw ?? ""}' is not an issue number.`);
    }
    const inflight = new Set(
      [...(context.values["inflight"] ?? "").matchAll(/\d+/g)].map((m): number => Number(m[0])),
    );
    const capRaw = context.values["cap"];
