...
| `assertion-results-no-rows.json` | a file entry with **no** `assertionResults` at all. It still claims the format — the `testResults` key is the whole sniff — so the refusal names the missing pointer instead of the useless "no format nen reads", which is the true sentence that tells a maintainer nothing about a document plainly meant to be this |
