...
| `kro-shaped/` | **two separate Gradle builds in one tree**, the shape the inventory found: the root is `gradle-android` (its own wrapper, both spellings, `settings.gradle.kts` including one application module and one library module) and `program/` is `compose-desktop` (its **own** wrapper and settings file, a `compose.desktop` block, included by nothing above it). Two lanes, `defaultLane: null`, the two agreeing `hosts` maps merged into one block, `{gw}` written per host and `{unitTestTask}` answered `:PlaceholderCore:test` from the settings file. The root's own build file *names* the Android plugin without applying it, so the marker reported is the module's — and the root's marker search stops at `program/`, because a directory shipping its own wrapper is its own build |
...
| `markers/gradle-android/`, `markers/compose-desktop/` | the same one marker each — but these two stacks' tool is a file the repository **commits**, so the marker tree carries the evidence for the row as well: `build`/`lint`/`ui-test` and `run` respectively are proposed with `{gw}` resolved from the host, and only `test` is withheld, because these trees have no settings file to read a module out of. Both also carry the note about the `gradle` toolchain row: its probe is `{gw} --version`, and a probe naming a pack token is one nen proposes no precondition for |
