...
| `gatsby-site/` | the `gatsby` stack end to end: the config marker, `gatsby` as a declared dependency, and a `scripts` block that answers `{archiveScript}` for `archive` and for the first step of the two-step `deploy`. The five rows the pack has no command for arrive as `unsupported` seats carrying its reasons |
