...
| `markers/gatsby/`, `markers/expo/`, `markers/xcode/`, `markers/gradle-android/`, `markers/compose-desktop/`, `markers/winui/` | one marker each for the six stacks whose commands are not confirmable from a `package.json` → the lane is proposed with `verbs: {}` and one reason per withheld row |
