...
/**
 * One `{exe, argv}` a declaration states, plus where its stdout goes.
 *
 ...
 * PRINTS the thing nen needs to read has, until now, had nowhere to put it: the
 * bundled Apple profile's own coverage row extracts a JSON report to stdout,
 * and `nen shu coverage` parses a FILE. A declaration naming `stdoutTo` says
 * "write this step's stdout there", and nen writes the bytes itself. No shell,
 * no redirection operator, no second process: the seam already captures the
 * child's stdout, and this is where those bytes land.
 *
...
export interface DeclaredStep {
  ...
  /** Repo-relative file this step's stdout is written to, or null. */
...
export type Invocation =
  ...
  | {
      readonly kind: "steps";
      readonly steps: readonly DeclaredStep[];
      readonly why: string | null;
      readonly raw: Readonly<Record<string, unknown>>;
    }
  | {
...
export function parseInvocation(path: string, pointer: string, value: unknown): Invocation {
  ...
  if (hasSteps) {
    ...
    return {
      kind: "steps",
      steps: steps.map((step, index): DeclaredStep => {
