...
describe("renameLabels -- idempotent, associations preserved by construction", () => {
  it("renames a label that still carries its old name", () => {
    ...
    const results = renameLabels(seams, TARGET, [{ from: "old", to: "new" }], false);
    expect(results[0]?.status).toBe("renamed");
  });

  it("is idempotent: a mapping already applied is reported done, never retried", () => {
    const seams = new ScriptedSeams([{ match: `gh ${listLabelNamesArgv(TARGET).join(" ")}`, result: listResult(["new"]) }]);
    const results = renameLabels(seams, TARGET, [{ from: "old", to: "new" }], false);
  ...
  it("fails when neither the old nor the new name exists", () => {
    const seams = new ScriptedSeams([{ match: `gh ${listLabelNamesArgv(TARGET).join(" ")}`, result: listResult([]) }]);
    const results = renameLabels(seams, TARGET, [{ from: "old", to: "new" }], false);
    expect(results[0]?.status).toBe("failed");
  });

  it("dry-run logs the call and never renames", () => {
    const seams = new ScriptedSeams([{ match: `gh ${listLabelNamesArgv(TARGET).join(" ")}`, result: listResult(["old"]) }]);
    const results = renameLabels(seams, TARGET, [{ from: "old", to: "new" }], true);
