...
export const parseCommand: Command = {
  ...
  run(context: CommandContext): number {
    ...
    // Typed explicitly: an unannotated `let` here would be an evolving `any`,
    // and the whole point of the parseTemplate/parseInvocation seam is that a
    // template's shape stays checked between the two calls.
    let grammar: Grammar;
    try {
