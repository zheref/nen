...
describe("the antigravity row", () => {
  it("mirrors skills and personas with the documented keys and the /name spelling", async () => {
    ...
    expect(json(result)["written"]).toEqual(["agents/_shared.md", "agents/scout.md", "skills/alpha/SKILL.md", "skills/beta/SKILL.md"]);
    ...
    expect(json(check)["ok"]).toEqual(["agents/_shared.md", "agents/scout.md", "skills/alpha/SKILL.md", "skills/beta/SKILL.md"]);
    expect(json(result)["skippedAgents"]).toEqual([]);
    expect(json(result)["includes"]).toEqual(["_shared.md"]);
    // The include travels beside the personas (S11): marker, body verbatim, no persona-shaped refusal for its missing frontmatter.
    const include = readFileSync(join(out, "agents", "_shared.md"), "utf8");
    expect(include.startsWith("<!-- GENERATED by nen surface mirror (surface: antigravity)")).toBe(true);
    expect(include).toContain("This file is pulled into every persona by reference.");
  });
});
...
describe("--hooks", () => {
  it("writes the row's manifest with the events renamed, in the universe", async () => {
    ...
    expect(json(result)["written"]).toEqual(expect.arrayContaining(["hooks.json", "hooks/bell.hook", "hooks/guard.hook"]));
    ...
    expect(statSync(join(out, "hooks", "bell.hook")).mode & 0o777).toBe(0o755);
    expect(readFileSync(join(out, "hooks", "bell.hook"), "utf8").split("\n").slice(0, 2)).toEqual([
      "#!/bin/sh",
      "# GENERATED by nen surface mirror (surface: antigravity) -- do not edit; edit the source and regenerate",
    ]);
    expect(readFileSync(join(out, "hooks", "guard.hook"), "utf8").startsWith("# GENERATED by nen surface mirror")).toBe(true);
    expect(readFileSync(join(out, "hooks.json"), "utf8")).toContain("${CLAUDE_PLUGIN_ROOT}/hooks/bell.hook");
    ...
    expect(json(again)["deleted"]).toEqual(["hooks.json", "hooks/bell.hook", "hooks/guard.hook"]);
  });
...
describe("check --installed", () => {
  ...
  it("exits 0 on a fresh install and 1 naming the drifted file after a byte changes", async () => {
    ...
    expect(json(fresh)["ok"]).toEqual(["agents/_shared.md", "agents/scout.md", "alpha/SKILL.md", "beta/SKILL.md"]);
  ...
  it("compares a claude-code plugin tree verbatim: the source IS the generation", async () => {
    ...
    expect(fresh.code).toBe(0);
    // The shared include is in the universe like any persona file (S11): verbatim here, so ok.
    expect(json(fresh)["ok"]).toEqual(["agents/_shared.md", "agents/scout.md", "skills/alpha/SKILL.md", "skills/beta/SKILL.md"]);
