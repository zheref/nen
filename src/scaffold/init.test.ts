...
describe("scaffoldInit", () => {
  it("creates every requested directory that does not exist", () => {
    const root = tempRoot();
    const result = scaffoldInit({ root, directories: ["src", "tests"], hook: HOOK, stack: STACK });
  ...
  it("is idempotent -- a second run creates nothing new for existing directories", () => {
    const root = tempRoot();
    scaffoldInit({ root, directories: ["src"], hook: HOOK, stack: STACK });
    const second = scaffoldInit({ root, directories: ["src"], hook: HOOK, stack: STACK });
    expect(second.createdDirectories).toEqual([]);
  });

  it("writes the commit-msg hook at the default path", () => {
    const root = tempRoot();
    const result = scaffoldInit({ root, directories: [], hook: HOOK, stack: STACK });
  ...
  it("is idempotent for the hook -- a second run with the same spec reports 'unchanged', not 'installed'", () => {
    const root = tempRoot();
    scaffoldInit({ root, directories: [], hook: HOOK, stack: STACK });
    const second = scaffoldInit({ root, directories: [], hook: HOOK, stack: STACK });
  ...
  it("--force replaces a differing hook, but backs up the original first", () => {
    ...
    const result = scaffoldInit({ root, directories: [], hook: HOOK, stack: STACK, force: true });
  ...
  it("takes a caller-supplied hook path", () => {
    const root = tempRoot();
    const result = scaffoldInit({ root, directories: [], hook: HOOK, stack: STACK, hookPath: "custom/hooks/commit-msg" });
  ...
  it("writes a canon-values template only when a path is given, and never overwrites an existing one", () => {
    const root = tempRoot();
    const withoutPath = scaffoldInit({ root, directories: [], hook: HOOK, stack: STACK });
    expect(withoutPath.canonValuesWritten).toBeNull();

    const result = scaffoldInit({ root, directories: [], hook: HOOK, stack: STACK, canonValuesPath: "canon-values.yml", scenario: "scenario-x" });
    ...
    scaffoldInit({ root, directories: [], hook: HOOK, stack: STACK, canonValuesPath: "canon-values.yml", scenario: "scenario-x" });
