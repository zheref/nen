...
```bash
nen="$(bash nen-bootstrap.sh --ref v0.14.1)" && "$nen" --version
nen bootstrap --ref v0.14.1 --source zheref/nen --script ./nen-bootstrap.sh
```

Once the `v0.14.1` release and its assets are published, the first prints
`0.14.1`. Before the tag exists, a fresh install's `curl` command in
