...
```
curl -fsSL https://raw.githubusercontent.com/zheref/nen/v0.14.3/bootstrap/nen.sh -o nen-bootstrap.sh
bash nen-bootstrap.sh --ref v0.14.3
```
...
```
nen="$(bash nen-bootstrap.sh --ref v0.14.3)"
"$nen" --version
```
...
```bash
nen="$(bash nen-bootstrap.sh --ref v0.14.3)" && "$nen" --version
nen bootstrap --ref v0.14.3 --source zheref/nen --script ./nen-bootstrap.sh
```

Once the `v0.14.3` release and its assets are published, the first prints
`0.14.3`. Before the tag exists, a fresh install's `curl` command in
