...
```
curl -fsSL https://raw.githubusercontent.com/zheref/nen/v0.13.1/bootstrap/nen.sh -o nen-bootstrap.sh
bash nen-bootstrap.sh --ref v0.13.1
```
...
```
nen="$(bash nen-bootstrap.sh --ref v0.13.1)"
"$nen" --version
```
...
```bash
nen="$(bash nen-bootstrap.sh --ref v0.13.1)" && "$nen" --version
nen bootstrap --ref v0.13.1 --source zheref/nen --script ./nen-bootstrap.sh
```

Once the `v0.13.1` release and its assets are published, the first prints
`0.13.1`. Before the tag exists, a fresh install's `curl` command in
...
108 verbs outside the binary — each one's purpose, arguments, exit codes and
...
- **Readiness & pull requests** — `pr` (ready, staleness, body-check, fetch,
  ...
  open), `gate`, `split`, `wc` (classify, squash, catch-up, publish, worktrees,
  swap), `stage`
