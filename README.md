...
bash nen-bootstrap.sh --ref v0.14.0
```
...
```
nen="$(bash nen-bootstrap.sh --ref v0.14.0)"
"$nen" --version
```
...
```bash
nen="$(bash nen-bootstrap.sh --ref v0.14.0)" && "$nen" --version
nen bootstrap --ref v0.14.0 --source zheref/nen --script ./nen-bootstrap.sh
```

Once the `v0.14.0` release and its assets are published, the first prints
`0.14.0`. Before the tag exists, a fresh install's `curl` command in
