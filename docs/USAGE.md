...
with the `nen` spelling. This document covers the **v0.7.0 line** (one new
verb, `loop iterate`, and no new family — the rest is the twenty-issue backlog
sweep, so what else is new in it is `stage triage`'s `local-config` and
`large` detectors, `pr ready` reading `gates.json`'s `dependabot_carve_out`,
one path base for every relative path flag, a single-quoted `gh api --jq` read
that classifies, and five refusals that moved to exit 2): 37 command
...
```bash
curl -fsSL https://raw.githubusercontent.com/zheref/nen/v0.7.0/bootstrap/nen.sh -o nen-bootstrap.sh
bash nen-bootstrap.sh --ref v0.7.0
```
...
```bash
nen="$(bash nen-bootstrap.sh --ref v0.7.0)"
"$nen" --version
```
...
```bash
nen bootstrap --ref v0.7.0 --source zheref/nen
```
```text
/home/me/.cache/nen/zheref_nen/v0.7.0/nen-linux-x64
```
...
```bash
nen bootstrap --ref v0.7.0 --source zheref/nen --script ./nen-bootstrap.sh
```
