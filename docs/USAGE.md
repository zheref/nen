...
with the `nen` spelling. This document covers the **v0.5.0 line** (the `surface`
family, `wc squash`, `pr`/`issue edit-body` and `commit check` are new in it,
and the legacy `schemas/` read fallback is gone from it): 37 command
...
```bash
nen bootstrap --ref v0.5.0 --source zheref/nen
```
```text
/home/me/.cache/nen/v0.5.0/nen-linux-x64
```
