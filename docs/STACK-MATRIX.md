...
| ✓ delegates to ... | the verb's work is another verb's, named -- only `warmup` does this, and only for its verification half |
...
| `coverage` | `{pm} --filter {package} test:coverage` | AN EXISTING COMMAND, NOT A PROPOSAL, in both repositories -- and both run the identical shape once per workspace package that declares a `test:coverage` script, each resolving to `vitest run --coverage` with a real `coverage: { provider: 'v8', reporter: ['text','lcov'] }` block. Cited verbatim: `{pm} --filter @kro/core test:coverage` then `{pm} --filter @kro/app test:coverage`; `{package}` is templated because the package names are those repositories', not this stack's. GAP WORTH CARRYING: neither repository's `test-coverage` target reaches `apps/web`, whose own `test` script already bakes coverage in. *(source: kro-pwa/Makefile:71-73 (packages/core/package.json:19, packages/app/package.json:22); GymKai/Makefile:51-53)* |
...
| `pnpm` | `9.0.0` | `pnpm --version` | `first-semver-on-stdout` | `corepack` | ADVISORY ONLY: this is the minimum nen has been TESTED against, and it never contributes to an install argv. kro-pwa's own `packageManager` field pins 9.15.9, and that pin -- the declaration's, not this one -- is what `--install` would activate. *(source: kro-pwa/package.json (`packageManager`))* |
