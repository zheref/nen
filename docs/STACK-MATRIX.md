...
| --- | --- | --- | --- |
| `script` | `libs.versions.toml` | `buildSrc` | WHAT COUNTS AS APPLIED, AND WHAT COUNTS AS NEITHER. Four spellings are accepted and they are the four an Android build actually uses: `id("app.cash.paparazzi")`, `id 'app.cash.paparazzi'`, `apply plugin: 'app.cash.paparazzi'`, and `alias(libs.plugins.paparazzi)` -- whose id is not in the build file AT ALL. It is in `gradle/libs.versions.toml`, as `paparazzi = { id = "app.cash.paparazzi", ... }`, and the alias is followed only where that entry is there to follow. EVERY SPELLING IS READ WITH COMMENTS STRIPPED: a `// id("app.cash.paparazzi")` is a line somebody wrote to switch the plugin OFF, and reading it as evidence proposes a task the build no longer creates. AND THE ANSWER IS THREE-VALUED. A module applying a plugin nen cannot resolve to an id -- an `alias(...)` no catalogue answers, an `apply(...)`, an `apply from:` -- or a lane compiling its own build logic in `buildSrc/` MAY be applying this plugin through it, and nen cannot see that it does. That is neither evidence nor absence: the row is withheld either way, and the reason says WHICH of the two it was, because a maintainer fixes them differently. |
...
| comment syntax | alias catalogue | build logic nen does not read | why |
| --- | --- | --- | --- |
| `script` | `libs.versions.toml` | `buildSrc` | FOUR SPELLINGS COUNT AS APPLIED, and they are the four an Android build uses: `id("app.cash.paparazzi")`, `id 'app.cash.paparazzi'`, `apply plugin: 'app.cash.paparazzi'`, and `alias(libs.plugins.paparazzi)` -- whose id is not in the build file at all but in `gradle/libs.versions.toml`, and which is followed only where that entry is there to follow. All four are read with COMMENTS STRIPPED: a `// id("app.cash.paparazzi")` is a line somebody wrote to switch the plugin off. And the answer is THREE-VALUED: a module applying a plugin nen cannot resolve to an id, or a lane compiling its own build logic in `buildSrc/`, MAY be applying this plugin without saying so anywhere nen can read -- neither evidence nor absence, withheld either way, with the reason saying which. The reading is stated here in full rather than pointed at the `test` gate's copy of it: a reader who wants to know why `ui-test` was seated should find the whole answer in the rule that seated it. |
...
- TWO ROWS HERE ARE CONDITIONAL ON A PLUGIN, AND THE MARKERS CANNOT SEE IT. `test` and `ui-test` both run `verifyPaparazziDebug`, which is the Paparazzi plugin's own task; the markers that identify this stack confirm a wrapper, a settings file and the Android APPLICATION plugin, and none of the three says anything about Paparazzi. So both rows are gated by this profile's `crossChecks` and are proposed only where a MODULE of the target lane applies that plugin -- in any of the four spellings a build script uses, comments stripped, with an `alias(...)` followed through `gradle/libs.versions.toml`. Where the plugin is absent, or where nen cannot see through a convention plugin or a `buildSrc/` to tell, the row is a SEAT carrying the reason AND the row this repository should write instead. This is the same rule the rest of the pack follows and the one this stack was breaking: a task the project does not contain is a warning, never a proposal.
